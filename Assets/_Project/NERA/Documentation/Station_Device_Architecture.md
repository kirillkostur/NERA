...
### MaintainableObject и управляемые VFX

В поле `Cleaning Effect` укажите `ParticleEffectController` дочернего
`VFX_Clear`. Сам `ParticleSystem` напрямую в компоненты игровой логики больше не
назначается.

Для всех VFX, которыми управляет состояние игры:

- `Play On Awake` должен быть выключен;
- корневой GameObject эффекта остаётся активным в префабе;
- запуск, плавная остановка и немедленная очистка частиц выполняются через
  `ParticleEffectController`;
- временное выключение родительского объекта очищает частицы, но контроллер
  сохраняет запрос на воспроизведение и восстанавливает длительный эффект при
  следующем включении родителя.

Обычная очистка запускает `VFX_Clear`, плавно останавливает его после
завершения и немедленно очищает частицы при отмене. Developer-команда
`ОЧИСТИТЬ` вызывает мгновенную очистку состояния без проигрывания VFX.
...
С анимационного события `Start_Scan` и до события `End_Scan` дрон считается
физически отсутствующим на станции. В этот период нельзя взаимодействовать с
его объектом, запускать обслуживание, открывать режим апгрейда или менять его
состояние через обычный интерфейс терминала. Дрон также не получает загрязнение
от станционной песчаной бури. Событие `End_Scan` сразу возвращает все эти
возможности в обычный режим. Принудительные developer-команды управления
питанием могут обходить это ограничение намеренно, но массовый чит
`ЗАГРЯЗНИТЬ` также пропускает отсутствующий дрон.
...
### Декоративный экран физического терминала

Объект `Station_Terminal/Visual_3D` включён только при одновременном выполнении
двух условий:

- общая энергия станции находится в состоянии `Online`;
- система `Terminal` запрошена активной в `StationSystemsController`.

Отключение энергии или самого терминала сразу скрывает `Visual_3D` вместе с
декорациями станции и карты. При следующем включении сначала показывается
декорация станции; выбранная вкладка снова меняет её только после открытия
терминала.

Дочерний эффект
`Station_Terminal/Visual_3D/SM_Station_Mini_3D/VFX_Sandstorm_Mini` использует
`ParticleEffectController`: частицы запускаются при старте песчаной бури и
немедленно очищаются при любом её завершении. `Play On Awake` выключен, а
корневой GameObject остаётся активным. Если терминал или питание отключены,
эффект сохраняет синхронизацию с погодой и возобновляется вместе с `Visual_3D`,
если буря ещё продолжается.
