...
В соседнем поле обязательно указывается стабильный `Object Id`. Он должен
точно совпадать во всех связанных местах:

- `StationSystems_Default -> Object Id`;
- `EnergyBalance_Default -> Station Object Cutoffs -> Object Id`;
- `StationObjectIdentity -> Object Id` на физическом корне;
- `StationObjectIdentity -> Object Id` на корне 3D-превью в `MainScene`.

Сейчас используются `station_solar_01` для солнечной панели,
`station_antenna` для активной стадии антенны и `station_turret_01`–
`station_turret_02` для отдельных турелей. Тип/роль устройства не является
его идентификатором.
...
`MaintainableObject` хранит condition, износ, сохранение и события состояния,
но сам больше не является точкой взаимодействия. Физическое обслуживание и
ручной запуск настраиваются в `StationDeviceInteractable`:

- если устройство требует обслуживания, взаимодействие сначала восстанавливает
  condition;
- исправное выключенное устройство запрашивает включение через тот же
  `StationSystemsController.SetRequestedActive`, который использует терминал;
- успешный ручной запуск сразу обновляет переключатель терминала;
- без питания попытка запуска не меняет запрошенное состояние;
- у уже работающего исправного устройства подсказка скрыта;
- `Press`/`Hold`, длительность и текст подсказки задаются в блоке Prompt самого
  `StationDeviceInteractable`.

Тип и ID не дублируются в этих компонентах: оба получают их через ближайший
родительский `StationObjectIdentity`. Полная схема физического объекта,
`Stage_N` и HUD-превью описана в `Station_Device_Architecture.md`.
...
## 7. Библиотека и исследование предметов

### Предмет без исследования

Если `ItemData.Research Definition` не назначен, предмет не требует
лабораторного анализа. После первого успешного добавления в
`PlayerInventory` он автоматически становится известным библиотеке:

- `Equipment` отображается во вкладке оборудования;
- `EngineeringPart` отображается во вкладке деталей;
- остальные известные типы распределяются по своей категории.

`Anomaly` не открывается таким способом: аномалии должны проходить анализ.
Все способы получения предмета, включая физический подбор, награды и
developer cheats, должны использовать `PlayerInventory.AddItem`, чтобы не
обходить регистрацию библиотеки.
