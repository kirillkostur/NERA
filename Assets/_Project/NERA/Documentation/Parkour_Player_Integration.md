...
- `Tab` открывает и закрывает обычный инвентарь;
- `C` запускает скольжение только при движении вперёд со скоростью не меньше
  `1 м/с`; из положения стоя действие не начинается;
- скольжение разрешено только к точке на той же высоте или ниже, поэтому
  наклонённая вверх поверхность не поднимает игрока;
...
отклоняет их, засоряет Console предупреждениями и может визуально задерживать
переход между braced hang и free hang.
