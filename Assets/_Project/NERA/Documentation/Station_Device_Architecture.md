...
- перед включением `VirtualCamOrbit` её орбита автоматически выбирает точку,
  ближайшую к текущей камере игрока; поэтому вход начинается с той же стороны
  объекта и Cinemachine плавно смешивает камеры без резкого перелёта через объект;
...
- при выходе объектная камера остаётся в выбранном игроком ракурсе;
- игровая `FreeLookCam` подстраивает свои орбитальные оси под текущую позицию и
  направление объектной камеры, после чего Cinemachine начинает смешивание;
- управление игроком разблокируется только после окончания обратного перехода;
- исходная позиция и угол объектной камеры не запоминаются и не
  восстанавливаются: при следующем входе орбита снова рассчитывается от текущей
  камеры игрока.
...
3. Пустой `Fake` становится видимым только тогда, когда для этого конкретного
   объекта и слота есть совместимая деталь в инвентаре или на складе.
...
Когда заполнены все `Physical Upgrade Slots` объекта, режим апгрейда
автоматически закрывается. Последующие короткие нажатия `E` не открывают его,
а подсказка апгрейда больше не отображается. Ремонт и запуск по удерживанию `E`
остаются доступны, если полностью улучшенный объект сломан или выключен.
